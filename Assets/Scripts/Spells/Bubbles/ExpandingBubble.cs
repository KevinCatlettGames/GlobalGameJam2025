        ...
        if (other.CompareTag("Player"))
        {
            ...
            if (player != null)
            {
                if (GameManager.Instance.PlayingLocal)
                    player.ApplyKnockbackLocal(OwnerID.Value, direction, knockbackRatio * currentSize, damageRatio * currentSize, isCrit);
                else
                    player.ApplyKnockbackServerRpc(OwnerID.Value, direction, knockbackRatio * currentSize, damageRatio * currentSize, isCrit);
            }
            Pop();
        }
        else if (other.CompareTag("Bubble"))
