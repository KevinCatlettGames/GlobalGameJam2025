...
public class GameManager : NetworkBehaviour
{
    ...
    {
        if (SceneManager.GetActiveScene().name == "LVL_Testing" || !AchievementSaveSystem.instance || !players[hitReferences[killerID].playerHitID].WasSlowedWhenLastHit) return;
    ...
    {
        if (TransportSwitcher.Instance && TransportSwitcher.Instance.isUsingRelay && NetworkManager.Singleton.LocalClientId != (ulong)killerID
            || !AchievementSaveSystem.instance || hitReferences[killerID].playerHitID != 1 || SceneManager.GetActiveScene().name != "LVL_Testing") return;
